...
def lambda_handler(event, context):
    ...
    # DynamoDB expects ISO 8601 format
    items_to_delete = table.scan(
        FilterExpression="item_timestamp <= :deleted_time",
        ExpressionAttributeValues={":deleted_time": deleted_time.isoformat() + "Z"},
    ).get("Items", [])
    ...
    for item in items_to_delete:
        ...
        # logger.info(
        # f"Successfully published message to SNS. Message: {message}")
        logger.info(
